!==============================================================================!
  program Main
!------------------------------------------------------------------------------!
!  This was the first attempt to write a Fortran program which would use       !
!  OpenACC for Graphical Processing Unit (GPU) acceleration.  It is very       !
!  simple.  Three matrices (a, b and c) are defined on the Central Processing  !
!  Unit (CPU) (in GPU computing jargon also called the "host"), then they are  !
!  coppied to the GPU (in GPU jargon also called the "device").  Once on the   !
!  "device", they are summed up (c = a + b) in a double loop, and the data     !
!  are coppied back to the "host" from where they are printed.                 !
!                                                                              !
...
!  result matrix (c) are passed to "device".  Matrics a and b are coppied with !
!  the command "!$acc data copyin(a, b)", because they are not copped back to  !
!  the "host" after the calculations.  Contrary to that, matrix "c" will hold  !
!  the result of the operation on the "device" and we want its contents back   !
!  once the calculation is over.  Therefore, we copy it with the command       !
!  "!$acc data copy(c)!", which means that data wil be coppied back once the   !
!  command "!$acc end data" is reached.                                        !
!                                                                              !
